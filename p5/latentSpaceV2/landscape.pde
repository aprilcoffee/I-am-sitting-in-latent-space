...
void modeLandscapeData() {
  ...
  if (modeT == 0) {
    ...
    image(gradientMappingRenderer, 0, 0, width/2, height);
    ...
    //gradientMappingRenderer.image(bg, 0, 0, width, height); // Replace 'bg' with your background image
    gradientMappingShader.set("img", CachedLandscape(landscapeCounter/2));
    //landscapeCounter++;

    //landscapeCounter = floor(map(MD[0], 0, 1, 0, 250));
    gradientMappingShader.set("landscape", CachedLandscape(landscapeCounter/2));
    gradientMappingShader.set("tempo", tempo);
    gradientMappingShader.set("modeT", modeT);
    colorSet = map(volume, 0, 10, 0, 1);
    if (tempo%4==0) {
      gradientMappingShader.set("ch1", 1.0);
    } else {
      gradientMappingShader.set("ch1", 0.0);
    ...
    image(gradientMappingRenderer, width/2, 0, width/2, height);
